...
jalBot이 의미 기반으로 반환한 결과는 사용하지 않습니다. 응답의 `matchType`이
`exact`이고 현재 검색어가 제목에 그대로 포함된 결과만 후보에 추가합니다.
같은 조건을 캐시 로드와 실제 순환 선택 직전에도 다시 검사하며, 검색어 배열이
변경되면 기존 캐시는 자동 무효화됩니다.
한글 검색어는 부분 문자열이 아니라 독립된 단어로 일치해야 하므로 `"멍"` 검색이
`"콧구멍"` 제목을 잘못 통과시키지 않습니다.
