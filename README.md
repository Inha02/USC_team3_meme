...
웹캠 프리뷰를 노출하지 않고 표정을 계속 분석하다가, 60×60px 플로팅 버튼을 누른 순간의 표정으로 한국어 밈을 만드는 Electron 앱입니다. MediaPipe 추론은 렌더러에서, jalBot 검색·LLM 호출과 파일 저장은 Electron 메인 프로세스에서 처리합니다.

## 먼저 확인할 edge-case

- 카메라 권한이 거부되거나 카메라가 다른 앱에서 사용 중이면 버튼이 회색 오류 상태가 됩니다.
- 얼굴 데이터가 한 프레임도 없으면 무표정으로 오판하지 않고 얼굴을 보여 달라는 안내를 표시합니다.
- 버튼을 누른 순간의 웹캠 프레임을 좌우 반전 JPEG로 캡처해 결과 창에서 표정과 함께 보여줍니다. 얼굴 이미지는 LLM이나 jalBot으로 전송하지 않습니다.
...
- LLM 키가 없거나 호출이 실패하면 카테고리별 내장 문구 중 하나를 무작위로 사용합니다.
...
```dotenv
LLM_API_KEY=숙명여대_Gateway_키
LLM_BASE_URL=https://factchat-cloud.mindlogic.ai/v1/gateway
LLM_MODEL=claude-sonnet-4-6
```

키가 없어도 로컬 폴백만으로 전체 UI 흐름을 확인할 수 있습니다. `.env`는 Git에 포함되지 않습니다.

숙명여대 Gateway 대신 공식 OpenAI API를 직접 사용하려면 `LLM_*` 값을 비우고
`OPENAI_API_KEY`와 `OPENAI_MODEL`을 설정합니다. Gateway 키는 OpenAI 공식 키와
형식이 다를 수 있으며, `LLM_API_KEY`에 넣으면 됩니다.
...
비디오와 유효하지 않은 이미지 URL을 제거한 뒤, 중복 제거 및 조회수 정렬을
수행합니다. 표정별 후보는 `jjalBotMap.json`으로 24시간 캐시되며 캡처와
다시 생성 때 라운드로빈으로 순환합니다.

선택한 jalBot 이미지 또는 GIF의 첫 프레임을 Sharp로 읽고 운영체제의 한글
폰트로 캡션을 합성합니다. 완성 JPEG는 사용자 데이터 폴더의 `generated`
캐시에 저장됩니다. jalBot 검색 API에는 별도의 API 키가 필요하지 않습니다.
...
`src/services/captionService.js`가 OpenAI 호환 Chat Completions API로 반말·15자 이하 단일 캡션을 요청합니다. 숙명여대 Gateway와 공식 OpenAI API를 모두 지원합니다. 출력은 줄바꿈과 따옴표를 제거하고 15자로 제한하며, 키가 없으면 즉시 내장 문구를 사용합니다.
...
1. `.env` 없이 실행하고 카메라를 허용합니다. 얼굴을 보인 뒤 버튼을 누르면 로컬 폴백 밈과 기본 캡션이 결과 창에 나와야 합니다.
...
8. LLM 키만 제거합니다. jalBot 짤은 선택되되 내장 한국어 캡션을 사용해야 합니다.
9. 다운로드를 누르고 취소합니다. 오류 문구 없이 결과 창이 유지되어야 합니다.
10. 네트워크를 끊고 다시 생성을 누릅니다. 15초 안에 폴백 결과로 복구되어야 합니다.
